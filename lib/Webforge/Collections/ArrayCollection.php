...
class ArrayCollection extends \Doctrine\Common\Collections\ArrayCollection implements Collection {
  ...
  protected function _getJSONFields() { // ist eher so zu Interface zwecken da
    return array();
  }
   ...
   */
  public function _deleteDiff(Collection $collection, $compareFunction) {
   ...
   */
  public function _insertDiff(Collection $collection, $compareFunction) {
    return new ArrayCollection(array_merge(
      $this->difference($collection->toArray(), $this->toArray(), $compareFunction)
    ));
  }
  
  protected function _difference(Array $c1, Array $c2, $compareFunction) {
  ...
  protected function _compileCompareFunction($compareFunction) {
    if ($compareFunction === self::COMPARE_TOSTRING) {
   ...
   * @return bool
   */
  public function _isSame(Collection $collection, $compareFunction) {
   ...
   * @return bool
   */
  public function _isEqual(Collection $collection, $compareFunction) {
    // ungleiche Anzahl ist nie gleich => fastcheck
    if (count($this) !== count($collection)) return FALSE;
   ...
   */
  public function _computeCUDSets(Collection $collection, $compareFunction, $uniqueHashFunction = NULL) {
    $compareFunction = $this->compileCompareFunction($compareFunction);
